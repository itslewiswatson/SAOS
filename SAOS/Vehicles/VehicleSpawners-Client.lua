...
function VehicleSpawners.UseSpawner(player,dim)
	if player == localPlayer and dim and not isPedInVehicle(localPlayer) then
		...
		if data then
			VehicleSpawners.Window = guiCreateWindow(Core.ResX/2-150,Core.ResY/2-175,300,350,"Vehicle Spawner",false)
			...
			guiGridListAddColumn(vehGrid,"Vehicle",0.9)
			local spawnButton = guiCreateButton(10,235,280,50,"Spawn",false,VehicleSpawners.Window)
			...
			local closeButton = guiCreateButton(10,290,280,50,"Close",false,VehicleSpawners.Window)
